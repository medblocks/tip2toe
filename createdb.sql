-- Reference: https://github.com/ehrbase/ehrbase/blob/develop/base/db-setup/createdb.sql
...
-- default user and db. Override in CLI using --set=user="$PGUSER" --set=db="$PGDATABASE"
\set user `echo $PGUSER`
\set db `echo $PGDATABASE`

CREATE SCHEMA IF NOT EXISTS ehr AUTHORIZATION :user;
CREATE SCHEMA IF NOT EXISTS ext AUTHORIZATION :user;
...
ALTER DATABASE :db SET search_path TO "$user",public,ext;
-- ensure INTERVAL is ISO8601 encoded
alter database :db SET intervalstyle = 'iso_8601';

GRANT ALL ON ALL FUNCTIONS IN SCHEMA ext TO :user;
